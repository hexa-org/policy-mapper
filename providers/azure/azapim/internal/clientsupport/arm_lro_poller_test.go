...
import (
	...
	"github.com/hexa-org/policy-mapper/providers/azure/azapim/internal/clientsupport"
...
func (m *MockHttpClient) addRequest(req *http.Request, _ *http.Response) {
	rKey := reqKey(req)
	count, found := m.requests[rKey]
	if !found {
		// First request is 0
		m.requests[rKey] = 0
	} else {
...
func (m *MockHttpClient) httpReqMatcher(expReq *http.Request, num int) interface{} {
	...
	return mock.MatchedBy(func(req *http.Request) bool {
		...
		rKey := reqKey(req)
		fmt.Println("httpReqMatcher m.requests", m.requests)
		if m.requests[rKey] != num {
			fmt.Println("httpReqMatcher reqNum DOES NOT MATCH", "num", num, "rKey", rKey)
			return false
		}
