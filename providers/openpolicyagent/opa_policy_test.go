...
import (
	...
	"github.com/hexa-org/policy-mapper/providers/openpolicyagent/decisionsupport"
...
func shouldAllow(t *testing.T, provider decisionsupport.OpaDecisionProvider, action string, principal string) {
	assert.True(t, allows(provider, action, principal))
}

func shouldNotAllow(t *testing.T, provider decisionsupport.OpaDecisionProvider, action string, principal string) {
	assert.False(t, allows(provider, action, principal))
}

func allows(provider decisionsupport.OpaDecisionProvider, action string, principal string) bool {
	allow, _ := provider.Allow(decisionsupport.OpaQuery{Input: map[string]interface{}{"method": action, "principal": principal}})
	return allow
}

func startCmd(cmd *exec.Cmd, _ int) {
