...
import (
	...
	filter2 "github.com/hexa-org/policy-mapper/internal/filter"
)
...
func ParseConditionRuleAst(condition ConditionInfo) (*filter2.Expression, error) {
	return filter2.ParseFilter(condition.Rule)
}

func ParseExpressionAst(expression string) (*filter2.Expression, error) {
	return filter2.ParseFilter(expression)
}
...
func SerializeExpression(ast *filter2.Expression) string {

	return walk(*ast, false)
}

func checkNestedLogic(e filter2.Expression, op filter2.LogicalOperator) string {
	// if the child is a repeat of the parent eliminate brackets (e.g. a or b or c)

	switch v := e.(type) {
	case filter2.PrecedenceExpression:
		e = v.Expression
	}

	switch v := e.(type) {
	case filter2.LogicalExpression:
		if v.Operator == op {
			return walk(e, false)
		} else {
			return walk(e, true)
...
func walk(e filter2.Expression, isChild bool) string {
	switch v := e.(type) {
	case filter2.LogicalExpression:
	...
	case filter2.ValuePathExpression:
