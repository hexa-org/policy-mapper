...
import (
    ...
    parser2 "github.com/hexa-org/policy-mapper/pkg/hexapolicy/conditions/parser"
)
...
func ParseConditionRuleAst(condition ConditionInfo) (*parser2.Expression, error) {
    return parser2.ParseFilter(condition.Rule)
}

func ParseExpressionAst(expression string) (*parser2.Expression, error) {
    return parser2.ParseFilter(expression)
}
...
func SerializeExpression(ast *parser2.Expression) string {

    return walk(*ast, false)
}

func checkNestedLogic(e parser2.Expression, op parser2.LogicalOperator) string {
    // if the child is a repeat of the parent eliminate brackets (e.g. a or b or c)

    switch v := e.(type) {
    case parser2.PrecedenceExpression:
        e = v.Expression
    }

    switch v := e.(type) {
    case parser2.LogicalExpression:
        if v.Operator == op {
            return walk(e, false)
        } else {
            return walk(e, true)
...
func walk(e parser2.Expression, isChild bool) string {
    switch v := e.(type) {
    case parser2.LogicalExpression:
    ...
    case parser2.ValuePathExpression:
