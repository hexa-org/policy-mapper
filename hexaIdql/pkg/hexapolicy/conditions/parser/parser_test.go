...
func TestNegParseTests(t *testing.T) {
	...
	ast, err = ParseFilter("emails[type eq work] ew \"hexa.org\"")
	if err != nil {
		fmt.Println(err.Error())
		assert.EqualError(t, err, "invalid IDQL idqlCondition: Missing and/or clause")
	}
	assert.Nil(t, ast, "No idqlCondition should be parsed")
	...
	ast, err = ParseFilter("emails[type eq val].value eq work")
	if err != nil {
		fmt.Println(err.Error())
		assert.EqualError(t, err, "invalid IDQL idqlCondition: expecting space after ']' in value path expression")
	}
	assert.Nil(t, ast, "No idqlCondition should be parsed")
