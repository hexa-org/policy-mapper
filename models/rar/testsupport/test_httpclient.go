...
func NewMockHttpClientWithJwt(jwksUrl string, issuer string, aud string, scopes []string) (*MockHTTPClient, error) {
	_ = os.Setenv(oauth2support.EnvOAuthJwksUrl, jwksUrl)
	_ = os.Setenv(oauth2support.EnvJwtRealm, "test")
	_ = os.Setenv(oauth2support.EnvJwtAudience, aud)
	_ = os.Setenv(oauth2support.EnvJwtKid, issuer)
	_ = os.Setenv(oauth2support.EnvJwtAuth, "true")

	jwtAuth, err := oauth2support.NewResourceJwtAuthorizer()
	if err != nil {
		return nil, err
	}
...
func (m *MockHTTPClient) Do(req *http.Request) (*http.Response, error) {
	...
	if m.Auth != "" {
		if m.Authorizer != nil {
			...
			m.Authorizer.ValidateAuthorization(resp, req, m.Scopes)
	...
	return nil, fmt.Errorf("missing mock response for request - %s", methodAndUrl)
}
...
func sortedQueryUrl(origUrl string) string {
	...
	for pName := range sortedUrl.Query() {
